...
function parseWalkthroughArgs(args) {
  ...
  while (i < args.length) {
    ...
    switch (a) {
      ...
      case '--template': opts.template = args[++i]; i++; break
      case '--tokens': opts.tokens = args[++i]; i++; break
...
Options:
  ...
  --template <html>    video motion template    (default: templates/walkthrough/motion.html;
                       "portrait" selects the stock phone-tutorial look; must read RENDER_DATA
                       and expose __seek(p) and __ready)
  --tokens <file>      recolor the template      (JSON {"wt-accent":"#4d465f"} or raw CSS
                       declarations; errors if the template has no token placeholder)
...
export async function runWalkthrough(args) {
  ...
  if (opts.emit === 'interactive') {
    ...
    const r = await emitVideo(manifest, {
      ...
      template: opts.template, tokens: opts.tokens,
    })
    console.log(`\n✓ ${r.out}`)
