...
Options:
  ...
  --template <html>    video motion template    (default: templates/walkthrough/motion.html;
                       must read RENDER_DATA and expose __seek(p) and __ready)
...
export async function runWalkthrough(args) {
  ...
  if (opts.emit === 'interactive') {
    ...
    const r = await emitVideo(manifest, {
      out: opts.out, fps: opts.fps, scale: opts.scale, canvasW: opts.canvasW, canvasH: opts.canvasH, template: opts.template,
    })
    console.log(`\n✓ ${r.out}`)
