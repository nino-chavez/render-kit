...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Fit the stage to the viewport height as well as its width, so a portrait capture keeps the
     caption and Back/Next in view. 150px covers the title, controls and hint below the stage. */
  .player { width:min(1100px, 100%, calc((100svh - 150px) * ${width} / ${height})); }
  ...
  @media (prefers-reduced-motion: reduce) {
    .hotspot::after, .dot::after { animation:none; }
    .hotspot, .caption, .bar > i { transition:none; }
  }
</style>
</head>
<body>
